...
describe('TimerDisplay', () => {
  ...
  it('should show notes input when save button is clicked', async () => {
    ...
    // Notes input is now in the drawer, so we need to open the drawer first
    const drawerToggle = wrapper.find('.btn-drawer')
    await drawerToggle.trigger('click')
    await wrapper.vm.$nextTick()
    
    // Check that notes input exists in the drawer
  ...
  it('should save time with notes when save button in notes input is clicked', async () => {
    ...
    // Open drawer to access notes input
    const drawerToggle = wrapper.find('.btn-drawer')
    await drawerToggle.trigger('click')
    await wrapper.vm.$nextTick()
